...
@section('title', 'Nouvelle Note de Débit')
@section('description', 'Créer une nouvelle note de débit')
@section('content')
                    ...
                    <div>
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <h6><a href="{{ route('bo.purchases.debit-notes.index') }}"><i
                                        class="isax isax-arrow-left me-2"></i>{{ __('Notes de débit') }}</a></h6>
                        </div>
                        <div class="card">
                            <div class="card-body">
                                <h5 class="mb-3">{{ __('Nouvelle note de débit') }}</h5>
                                @if ($errors->any())
                                        ...
                                        <div class="col-lg-4 col-md-6">
                                            <div class="mb-3">
                                                ...
                                                @error('vendor_bill_id')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                                <small class="text-muted d-block mt-1"><i class="isax isax-info-circle me-1"></i>{{ __('Optionnel. Pour lier à une facture fournisseur,') }} <a href="{{ route('bo.purchases.vendor-bills.create') }}">{{ __('créez-en une d\'abord') }}</a>.</small>
                                            </div>
                                        </div>
                                        <div class="col-lg-4 col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text"
                                                    ...
                                                    value="{{ old('debit_note_date', date('d-m-Y')) }}">
                                    ...
                                    <div class="border-top pt-3 mb-3">
                                        ...
                                        <div class="mb-3">
                                            <div class="form-check form-switch">
                                                ...
                                                <label class="form-check-label" for="enable_tax">{{ __('Activer la taxe') }}</label>
                                            </div>
                                        </div>
                                        <div class="table-responsive rounded border-bottom-0 border mb-3">
                                            <table class="table table-nowrap add-table m-0" id="items-table" style="min-width: 550px;">
                                                ...
                                                <tbody class="add-tbody">
                                                    <tr class="item-row">
                                                        <td><input type="text" name="items[0][label]" class="form-control" value="{{ old('items.0.label') }}" placeholder="{{ __('Libellé') }}" required></td>
                                                        <td><input type="number" name="items[0][quantity]" class="form-control item-qty" value="{{ old('items.0.quantity', 1) }}" min="0.001" step="0.001" required></td>
                                                        <td><input type="number" name="items[0][unit_price]" class="form-control item-price" value="{{ old('items.0.unit_price', 0) }}" min="0" step="0.01" required></td>
                                                        <td class="tax-col">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleTax() {
            ...
            if (enabled && defaultTaxGroup) {
                document.querySelectorAll('.item-tax').forEach(sel => {
                    if (!sel.value) {
                        const val = defaultTaxGroup.id !== undefined ? (typeof defaultTaxGroup.rate !== 'undefined' ? 'cat_' + defaultTaxGroup.id : defaultTaxGroup.id) : '';
                        sel.value = val;
                    }
                });
            }
        ...
        addBtn.addEventListener('click', function() {
            ...
            row.innerHTML = `
                <td><input type="text" name="items[${itemIndex}][label]" class="form-control" placeholder="{{ __('Libellé') }}" required></td>
