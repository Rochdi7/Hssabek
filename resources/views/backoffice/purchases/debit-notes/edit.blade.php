...
@section('title', 'Modifier la Note de Débit')
@section('description', 'Modifier les détails de la note de débit')
@section('content')
            ...
            <div class="row">
                <div class="col-md-10 mx-auto">
                    <div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <h5 class="mb-3">{{ __('Modifier la note de débit') }} — {{ $debitNote->number }}</h5>
                                @if ($errors->any())
                                            ...
                                            <table class="table table-nowrap add-table m-0" id="items-table" style="min-width: 550px;">
                                                ...
                                                <tbody class="add-tbody">
                                                    @foreach(old('items', $debitNote->items->toArray()) as $i => $item)
                                                        ...
                                                        @endphp
                                                        <tr class="item-row">
                                                            <td><input type="text" name="items[{{ $i }}][label]" class="form-control" value="{{ $item['label'] ?? $item['product']['name'] ?? '' }}" placeholder="{{ __('Libellé') }}" required></td>
