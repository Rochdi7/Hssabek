<head>
    ...
    @if (!Route::is(['index']))
        <title>
            {{ implode(' ',collect(explode('-', Route::currentRouteName()))->reject(fn($word, $i) => $i === 0 && $word === 'ui')->map(fn($word) => ucfirst($word))->toArray()) }}
            | Kanakku - Invoice and Billing Management Admin Dashboard Template</title>
    @endif
