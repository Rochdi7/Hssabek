...
@section('title', 'Modifier le Bon de Livraison')
@section('description', 'Modifier les détails du bon de livraison')
@section('content')
    <!-- ========================
                Start Page Content
            ========================= -->
        ...
        <div class="content">
            ...
            <div class="row">
                <div class="col-md-11 mx-auto">
                    ...
                    <div class="card">
                        <form action="{{ route('bo.sales.delivery-challans.update', $deliveryChallan) }}" method="POST"
                            ...
                            <div class="card-body">
                                <div class="top-content">
                                    <div class="purchase-header mb-3">
                                        <h6>{{ __('Modifier le bon de livraison') }}</h6>
                                    </div>
                                    <div>
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <div class="input-group position-relative">
                                                                    <input type="text" name="challan_date"
                                                                        ...
                                                                        value="{{ old('challan_date', $deliveryChallan->challan_date instanceof \Carbon\Carbon ? $deliveryChallan->challan_date->format('d-m-Y') : $deliveryChallan->challan_date) }}"
                                                    ...
                                                    <tr class="item-row">
                                                        ...
                                                        <td>
                                                            <input type="number"
                                                                ...
                                                                value="{{ $item['quantity'] ?? 1 }}" min="0.001"
                                                                step="0.001" required>
                                                        </td>
                                                        <td>
                                                            <input type="number"
                                                                ...
                                                                <optgroup label="{{ __('Taux de taxes') }}">
                                                                    @foreach ($taxCategories as $tc)
                                                                        <option value="cat_{{ $tc->id }}" data-rate="{{ $tc->rate }}" data-type="category"
                                                                            {{ (isset($item['tax_group_id']) && $item['tax_group_id'] == 'cat_'.$tc->id) ? 'selected' : '' }}>
                                                                            {{ $tc->name }} ({{ $tc->rate }}%)
                                                                        </option>
                                                                    @endforeach
                                                                </optgroup>
                                                                ...
                                                                <optgroup label="{{ __('Groupes de taxes') }}">
                                                                    @foreach ($taxGroups as $tg)
                                                                        <option value="{{ $tg->id }}" data-rate="{{ $tg->rates->sum('rate') }}" data-type="group"
                                                                            {{ (isset($item['tax_group_id']) && $item['tax_group_id'] == $tg->id) ? 'selected' : '' }}>
