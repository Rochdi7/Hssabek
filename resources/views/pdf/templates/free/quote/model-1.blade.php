...
<div class="page">
    ...
    <table class="header-table">
        <tr>
            <td style="width: 60%;">
                @if($tenant)
                    ...
                    @if($logoPath && file_exists($logoPath))
                        <img src="{{ $logoPath }}" alt="logo" style="max-height:120px;max-width:250px;width:auto;height:auto;margin-bottom:8px;"><br>
                    @endif
                @endif
                @php
            ...
            <td style="width: 40%;">
                <div class="doc-title">{{ strtoupper($pdfDocumentTitle ?? 'Devis') }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="items-table">
        <thead>
            <tr>
                ...
                <th class="text-right" style="width: 10%;">MÉTRAGE</th>
        ...
        <tbody>
            ...
            @php $mode = $item->calculation_mode ?? 'quantity'; @endphp
            <tr>
                @if($hasMeasurement)
                <td>
                    {{ $item->label }}
                ...
                <td class="text-center">
                    @if(in_array($mode, ['surface','volume']) && $item->length)
                        {{ rtrim(rtrim(number_format($item->length, 3, ',', ' '), '0'), ',') }}
                    @endif
                </td>
                <td class="text-center">
                    @if($mode === 'surface')
                        {{ $item->height ? rtrim(rtrim(number_format($item->height, 3, ',', ' '), '0'), ',') : ($item->width ? rtrim(rtrim(number_format($item->width, 3, ',', ' '), '0'), ',') : '') }}
                    @elseif($mode === 'volume')
                        {{ $item->height ? rtrim(rtrim(number_format($item->height, 3, ',', ' '), '0'), ',') : '' }}
                    @endif
                </td>
                <td class="text-center">{{ rtrim(rtrim(number_format($item->quantity, 3, ',', ' '), '0'), ',') }}</td>
                <td class="text-right">
                    @if($item->calculated_measurement)
                        {{ rtrim(rtrim(number_format($item->calculated_measurement, 3, ',', ' '), '0'), ',') }} {{ $item->measurement_unit }}
                    @endif
                </td>
